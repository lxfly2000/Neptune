...
LAppModel.prototype.preloadMotionGroup = function (name) {
    ...
    for (var i = 0; i < this.modelSetting.getMotionNum(name); i++) {
        ...
        this.loadMotion(file, parsePath(this.modelHomeDir, file), function (motion) {
...
LAppModel.prototype.setFadeInFadeOut = function (name, no, priority, motion) {
    ...
    if (this.modelSetting.getMotionSound(name, no) == null) {
        ...
        var dialog;
        if (MyTools.dialogEnabled && (dialog = this.modelSetting.getMotionDialog(name, no)) !== null)
            MyTools.showDialog(dialog);

        var snd = document.createElement("audio");
        snd.src = parsePath(this.modelHomeDir, soundName);
        snd.volume = MyTools.volume;
        snd.addEventListener('ended', MyTools.hideDialog);
        MyTools.audio = snd;
